...
> 1. В чем разница между `apt update` и `apt upgrade`?
    > Update отвечает за обновления списка пакетов из доверенных репозиториев. Upgrade отвечает за обновление уже установленных пакетов

> Зачем в начале Bash-скрипта ставить `#!/bin/bash`?
    > Это интерпретатор (Shebang) который указывает что нужно выполнять код построчно без перевода на машинный язык

> Какой символ используется для перенаправления вывода команды в файл с полной перезаписью файла?
    >  для полной перезаписи файла используется символ `>`, если нужно дозаписать файл то мы используем `>>`
...
**Связанные заметки**: [[Topic Linux Fundamentals]], [[Topic Linux Permissions & Users]], [[Linux Services (Systemd)]]
