...
`sudo apt update` - отвечает за обновление списка из рипозитория `sudo apt upgrade -y` - отвечает за установку обновлений на уже установленные приложения. Ключ `-y` означает что мы согласны на установку(удобно когда нужно массово обновить приложения)
...
- `sudo apt remove <name>` — удалить пакет. [APT](https://web.obsidian.my.to/quartz/LearningDevOps/%D0%9D%D1%83%D0%B6%D0%BD%D1%8B%D0%B5-%D0%BA%D0%BE%D0%BC%D0%B0%D0%BD%D0%B4%D1%8B-%D0%B4%D0%BB%D1%8F-DevOps-Engineer#f35110)

### Создание скрипта

> Основы Bash
> 
> Скрипт начинается с **Shebang**: `#!/bin/bash`. Чтобы запустить файл как скрипт, нужны права на исполнение: `chmod +x script.sh`.
...
> 1. В чем разница между `apt update` и `apt upgrade`? > Update отвечает за обновления списка пакетов из доверенных репозиториев. Upgrade отвечает за обновление уже установленных пакетов

> Зачем в начале Bash-скрипта ставить `#!/bin/bash`? > Это интерпретатор (Shebang) который указывает что нужно выполнять код построчно без перевода на машинный язык

> Какой символ используется для перенаправления вывода команды в файл с полной перезаписью файла? > для полной перезаписи файла используется символ `>`, если нужно дозаписать файл то мы используем `>>`

### Чек-лист

- [x]  Умею обновлять систему через терминал.
- [x]  Знаю, как установить любой софт (например, Nginx).
- [x]  Написал и запустил свой первый `.sh` скрипт. 

**Связанные заметки**: [Topic Linux Fundamentals](https://web.obsidian.my.to/quartz/LearningDevOps/Topic-Linux-Fundamentals), [Topic Linux Permissions & Users](https://web.obsidian.my.to/quartz/LearningDevOps/Topic-Linux-Permissions--and--Users), [Linux Services (Systemd)](https://web.obsidian.my.to/quartz/LearningDevOps/Linux-Services-\(Systemd\))